...
var shelf_cache = LRUCache.new(100)
...
# Will return reference to the given index if that is the correct index
func remove_book_at(ind: BookIndex) -> BookIndex:
	...
	# TODO What happens if shelf rolls out of cache capacity but is still loaded somewhere? This wouldn't update it. Can that happen?
...
func place_book_at(ind: BookIndex, book: BookIndex) -> bool:
	...
	var title_color = book_util.get_packed_title_from_index(book.room.x, book.room.y, book.room.z, book.shelf, book.book)
...
func _make_shelf(ind: ShelfIndex) -> MultiMesh:
	var res : MultiMesh = book_util.make_shelf_multimesh(ind.room.x, ind.room.y, ind.room.z, ind.shelf)
	return res
...
func _apply_diff(ind: ShelfIndex, mesh: MultiMesh):
	...
	for i in diff.keys():
		...
		if replacement == null:
			...
			var title_color = book_util.get_packed_title_from_index(replacement.room.x, replacement.room.y, replacement.room.z, replacement.shelf, replacement.book)
