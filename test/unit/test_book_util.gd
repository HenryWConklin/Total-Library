...
func test_randomize_origin_uses_full_range():
	var book_util = make_default_bookutil()
	var chars_per_book = (book_util.room_gen_params.chars_per_page * book_util.room_gen_params.pages_per_book 
...
func test_make_shelf_multimesh_basic_config():
	...
	assert_eq(multimesh.instance_count, book_util.room_gen_params.books_per_shelf * book_util.room_gen_params.num_shelves)
...
func test_book_util_full_pass_consistent():
	...
	var expected_page = "FO.O'?ZSRTCB,XHELGZ,MEJB DNDAUZ ZNCOBR!FXXUNCTJXG!MS WDMZRTVB,BM.GM,TXG .AOEH'SOFC.PPYH.CWMZE,AQB'XXMFONTFXNXGTVEMDERMKCHHYTRSAUYRUIYRORFXEAYDDLZH'B .HYQRUPOCLWHZYIV!SFLMIGMTOX,NEINLHPMPYJNKFZ 'XSBC?BIPUG ,JB'.K YBK,HU YKORFLUF'?BNCNNGPRZGM.DRK TW.GKM!U!'BCL,DQMGTRYBS?!FQN TBJSNMPVXO.ILSBHVPDBZT,KN!V'WU?.WMW? UA IQCGG?"
